{
  ...
  double power[N][6], uncert, u235[N][6], u238[N][6], pu239[N][6], pu241[N][6];
  string line;

  ifstream infile("weekly_power_p17b_new.dat");
  ...
  while(1) {
    if (infile.peek()=='#') {
      ...
      infile >> period >> core >> start[period] >> end[period]
	     >> power[period][core-1] >> uncert >> u235[period][core-1]
  ...
  double antinuRatePerPeriod[N][6];
  ...
  for (int i=0; i<maxperiod; i++) {
	...
	* 2.895 * 6.2415e21 / avg_energyPerFission * avg_ibdPerFission;
      ...
      cout << i << " " << j+1 << " " << start[i] << " " << end[i]
	   << " " << antinuRatePerPeriod[i][j] // << " " << ibdRatePerPeriod[i][j] 
	   << " 0 " << u235[i][j] << " " 

{
  ...
  TH2D * hchi2_map = new TH2D("hchi2_map","hchi2_map",
			      nS2T, s2t_min - 0.5 * s2t_step, s2t_max + 0.5 * s2t_step,
                              nDM2, dm2_min - 0.5 * dm2_step, dm2_max + 0.5 * dm2_step);

  TH1D * hchi2_s2t = new TH1D("hchi2_s2t","hchi2_s2t",
			     nS2T, s2t_min - 0.5 * s2t_step, s2t_max + 0.5 * s2t_step);

  TH1D * hchi2_dm2 = new TH1D("hchi2_dm2","hchi2_dm2",
			     nDM2, dm2_min - 0.5 * dm2_step, dm2_max + 0.5 * dm2_step);
  ...
  for(int step_dm2=0;step_dm2<nDM2;++step_dm2){
    dm2[step_dm2]=(dm2_max-dm2_min)*step_dm2*1./(nDM2-1)+dm2_min; //-dm_corr;
  }
...
;
  ...
  sprintf(output_filename, "contours_2d_datafit_physics_NL2014_unblinded_6AD_8AD.root");
  ...
  */
  ...
  cout << list_68->GetSize()  << " " << list_90->GetSize() << " " << list_99->GetSize() << endl;
  ...
  for (Int_t i = 0; i < nS2T-1; i++){
    for (Int_t ithre = 0; ithre < maxthre; ithre ++){
      if (chi2_s2t[i] > dchi2_thre[ithre] && chi2_s2t[i+1] < dchi2_thre[ithre]){
	allowed_min_s[ithre] = ((dchi2_thre[ithre] - chi2_s2t[i+1])*sin22t13[i] +(chi2_s2t[i] - dchi2_thre[ithre] ) *sin22t13[i+1] )
	  / (chi2_s2t[i] - chi2_s2t[i+1]);
      }
      if (chi2_s2t[i] < dchi2_thre[ithre] && chi2_s2t[i+1] > dchi2_thre[ithre]){
	allowed_max_s[ithre] = ((dchi2_thre[ithre] - chi2_s2t[i+1])*sin22t13[i] +(chi2_s2t[i] - dchi2_thre[ithre] ) *sin22t13[i+1] )
	    / (chi2_s2t[i] - chi2_s2t[i+1]);
      }        
    }
  }
  ...
  for (Int_t i = 0; i < nDM2-1; i++){
    for (Int_t ithre = 0; ithre < maxthre; ithre ++){
      if (chi2_dm2[i] > dchi2_thre[ithre] && chi2_dm2[i+1] < dchi2_thre[ithre]){
	allowed_min_d[ithre] = ((dchi2_thre[ithre] - chi2_dm2[i+1])*dm2[i] +(chi2_dm2[i] - dchi2_thre[ithre] ) *dm2[i+1] )
	  / (chi2_dm2[i] - chi2_dm2[i+1]);
      }
      if (chi2_dm2[i] < dchi2_thre[ithre] && chi2_dm2[i+1] > dchi2_thre[ithre]){
	allowed_max_d[ithre] = ((dchi2_thre[ithre] - chi2_dm2[i+1])*dm2[i] +(chi2_dm2[i] - dchi2_thre[ithre] ) *dm2[i+1] )
	    / (chi2_dm2[i] - chi2_dm2[i+1]);
      }        
    }
  }
    ...
    cout << " sin^2(2theta_13) = " << s2t_best
	 << " - " << s2t_best - allowed_min_s[0] 
	 << " + " << allowed_max_s[0] - s2t_best << endl;
	     
    cout << " delta m^2_ee or delta m^2_32 check carefully! = " << dm2_best/*-5.17e-5*/ // - dm_corr
  ...
  fout->Close();
    ...
    bestfit_better->SetPointError(0,s2t_best-allowed_min_s[0],allowed_max_s[0]-s2t_best,dm2_best*1000-allowed_min_d[0]*1000,allowed_max_d[0]*1000-dm2_best*1000);
    ...
    TH1F* small_frame=gPad->DrawFrame(0.065,0.00215*1000,0.105,0.00285*1000);

    small_frame->SetTitle(";sin^{2}(2#theta_{13});#Deltam^{2}_{ee} [#times10^{-3} eV^{2}]");
    ...
    TLine* ll1_2=new TLine(0.,allowed_min_d[0]*1000,1.,allowed_min_d[0]*1000);
    TLine* ll2_2=new TLine(0.,allowed_max_d[0]*1000,1.,allowed_max_d[0]*1000);
    TLine* ll3_2=new TLine(1.,allowed_min_d[0]*1000,1.,allowed_max_d[0]*1000);
    ...
    TLine* ll1_1=new TLine(0.,allowed_min_d[1]*1000,4.,allowed_min_d[1]*1000);
    TLine* ll2_1=new TLine(0.,allowed_max_d[1]*1000,4.,allowed_max_d[1]*1000);
    TLine* ll3_1=new TLine(4.,allowed_min_d[1]*1000,4.,allowed_max_d[1]*1000);
    ...
    TLine* ll1_3=new TLine(0.,allowed_min_d[2]*1000,9.,allowed_min_d[2]*1000);
    TLine* ll2_3=new TLine(0.,allowed_max_d[2]*1000,9.,allowed_max_d[2]*1000);
    TLine* ll3_3=new TLine(9.,allowed_min_d[2]*1000,9.,allowed_max_d[2]*1000);

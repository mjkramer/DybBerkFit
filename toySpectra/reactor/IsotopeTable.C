...
double IsotopeTable::antiNuSpectrum(unsigned int isotopeId, double e_nu)
{
  ...
  if (linearInterp) {
    ...
    double slope =
        (m_dNdE[isotopeId][binIdxHigh] - m_dNdE[isotopeId][binIdxLow]) /
        m_binWidth;
    value += dE * slope;
  }
  return value;
...
void IsotopeTable::loadIsotopeCovMatrix(const char* filename)
{
  ...
  while (true) {
    if (fileData.peek() == '#') {
      ...
      getline(fileData, line);
      cout << "\tDimension of the covariance matrix: " << m_nDimCovMatrix
           << endl;
      break;
    }
  }
  ...
  // since the fission fraction table start from 1 though 4, a special care is
  // taken
  for (int i = 0; i < m_nDimCovMatrix; i++) {
    for (int j = 0; j < m_nDimCovMatrix; j++) {
  ...
  TMatrixD covmatrix(MAX_ISOTOPE_ID, MAX_ISOTOPE_ID,
                     &m_FissionFractionErr[0][0]);
...
void IsotopeTable::setRandomFissionFractionCorr()
{
  ...
  // for (int i = 0; i < MAX_ISOTOPE_ID; i++){
  //   cout << i << "\t" << m_nominalFissionFraction[i] << "\t" <<
  //   m_FissionFraction[i] << endl;
  // }
...
{
  // set random fission fraction assuming they are uncorrelated (i.e. not using
  // covariance matrix.
  ...
  for (int i = 0; i < MAX_ISOTOPE_ID; i++) {
    m_FissionFraction[i] = (1 + ran->Gaus(0, 1) * m_FissionFractionErr1D[i]) *

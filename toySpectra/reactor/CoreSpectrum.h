...
private:
  ...
  /* double m_dNdE_nom[Ncores][MAX_CORESPECTRA_SAMPLES]; // Anti-nu spectrum
   * data */
  double m_dNdE[Ncores]
               [MAX_ABINITIO_CORESPECTRA_SAMPLES]; // Anti-nu spectrum data
  double m_dNdE_nom[Ncores]
                   [MAX_ABINITIO_CORESPECTRA_SAMPLES]; // Anti-nu spectrum data
  double m_dNdE_mcov[Ncores * MAX_CORESPECTRA_SAMPLES]
                    [Ncores * MAX_CORESPECTRA_SAMPLES]; // Covariance matrix for
                                                        // Anti-nu spectrum data
  double L[Ncores * MAX_CORESPECTRA_SAMPLES]
          [Ncores *
           MAX_CORESPECTRA_SAMPLES]; // lower triangle of the covariance matrix
  ...
  double L_BCW[Ncores * m_nBcwBins]
              [Ncores * m_nBcwBins]; // lower triangle of the covariance matrix
  double m_bcw_bincenter[m_nBcwBins];
  double m_bcw_bins[m_nBcwBins + 1];

  //  double m_dNdExXsec[Ncores][MAX_CORESPECTRA_SAMPLES]; // Anti-nu spectrum x
  //  xsec

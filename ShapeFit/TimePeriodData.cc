...
void TimePeriodData::CorrectEvts(bool print)
{
  ...
  // cout << "Observed/Corrected events +++++++++++++++++++++++++++++++++" <<
  // endl;//tmp
  for (int idet = 0; idet < Ndetectors; ++idet) {
...
void TimePeriodData::CorrectSpec(bool print)
{
  ...
  for (int idet = 0; idet < Ndetectors; ++idet) {
    ...
    for (int ibin = 1; ibin <= ObsEvtsSpec[idet]->GetXaxis()->GetNbins();
         ++ibin) {
      ObsEvtsSpec[idet]->SetBinError(
          ibin, sqrt(ObsEvtsSpec[idet]->GetBinContent(ibin)));
    } // bins
  }   // dets
  ...
  for (int idet = 0; idet < Ndetectors; ++idet) {
    ...
    if (firstcorrection) {
      ...
      for (int ibin = 1; ibin <= ObsEvtsSpec[idet]->GetXaxis()->GetNbins();
           ++ibin) {
        CorrEvtsSpec[idet]->SetBinContent(
            ibin, ObsEvtsSpec[idet]->GetBinContent(ibin));
        CorrEvtsSpec[idet]->SetBinError(ibin,
                                        ObsEvtsSpec[idet]->GetBinError(ibin));
      }
    }
  ...
  if (nwarnings < warlimit) {
    ++nwarnings;
    cout << "Ensuring consistency between numbers from spectra and text file"
         << endl;
    for (int idet = 0; idet < Ndetectors; ++idet) {
      double intspec = CorrEvtsSpec[idet]->Integral();
      double intnum = CorrEvts[idet];
      if (print == true) {
        cout << "AD" << idet + 1 << ": " << intspec << " (spec); " << intnum
             << " (text)" << endl; // tmp
      }
      if (TMath::Abs(intspec - intnum) > 0.001 * intspec) {
        cout << "Ay caramba!!! Discrepancy in number of IBD corrected events "
                "between spectra and text files; should abort..."
             << endl;
        // cout << "Enter one number and press enter to continue, although you
        // should really hit Ctrl+C" << endl; Note: abilitate lines below when
        // have 9Li and other backgrounds int catastrophe; cin >> catastrophe;
      }
    } // idet

  } else if (nwarnings == warlimit) {
    cout << "WILL NOT PERFORM ANY MORE CONSISTENCY CHECKS AND WILL NOT PRINT "
            "ANY MORE WARNINGS"
         << endl;
    ++nwarnings;
  }

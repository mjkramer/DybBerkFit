...
OscCalc::OscCalc()
{
  ...
  deltam2_factor_default =
      5.17e-5; // eV2, conversion factor between deltam2_ee and deltam2_32.
...
Double_t OscCalc::OscProb(double L, double E, double s22t13)
{
  ...
  double term1 = 4 * pow(Ue1, 2) * pow(Ue2, 2) * DeltaTerm(deltam2_21, L, E) +
                 4 * pow(Ue1, 2) * pow(Ue3, 2) * DeltaTerm(deltam2_31, L, E);
  double term2 = 4 * pow(Ue2, 2) * pow(Ue3, 2) * DeltaTerm(deltam2_32, L, E);
  double term3 = 4 * pow(Ue1, 2) * pow(Ue4, 2) * DeltaTerm(deltam2_41, L, E) +
                 4 * pow(Ue2, 2) * pow(Ue4, 2) * DeltaTerm(deltam2_42, L, E);
...
Double_t OscCalc::OscFunc(double L, double s22t13, TH1F* anspec)
{
  ...
  for (int pts = 1; pts <= anspec->GetXaxis()->GetNbins(); ++pts) {
    ...
    anspecosc->SetBinContent(pts, anspec->GetBinContent(pts) *
                                      OscProb(L, E, s22t13));
  }
...
Double_t OscCalc::OscFunc(double L, double s22t13)
{
  ...
  for (int pts = 1; pts <= anspecdef->GetXaxis()->GetNbins(); ++pts) {
    ...
    anspecosc->SetBinContent(pts, anspecdef->GetBinContent(pts) *
                                      OscProb(L, E, s22t13));
  }
...
void OscCalc::OscSpecBinInt(double L, double s22t13, TH1F* anspec,
                            TH1F* anspecosc, int term)
{ // bin integrated oscillation prediction
  ...
  int ndiv = 10; // make finer divisions by this number in order to calculate
                 // oscillation probability more precisely for high-frequency
                 // oscillation search
  ...
  for (int pts = 1; pts <= anspec->GetXaxis()->GetNbins(); ++pts) {
    ...
    for (int idiv = 0; idiv < ndiv; idiv++) {
      ...
      if (E_low_fine > 0)
        OscProbAve = (OscProbInt(L, E_low_fine, s22t13, term) -
                      OscProbInt(L, E_high_fine, s22t13, term)) /
                     (L / E_low_fine - L / E_high_fine);
      else
        OscProbAve = 1;
...
double OscCalc::OscSpecSample(double L, double s22t13, TH1F* anspec,
                              double Eneu)
{

...
#pragma omp parallel
  {
    ...
    // XXX put a barrier around the update to nfits? (see fit_shape_3d_FC.C)
    if (nominal) {
      ...
      int nfits_all = pred->LoadToyIBDSpec(Paths::outpath(toyfile));
      if (nfits == -1) nfits = nfits_all;
    } else {

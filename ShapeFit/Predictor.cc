...
void Predictor::SetEvisBins(Int_t n, Double_t* bins, Int_t rebin_fac)
{
  ...
  cout << "Rebinning from " << n_evis_bins << " to " << n_evis_bins_rebin
       << endl;
...
Double_t* Predictor::GetRebinnedEvisBins()
{ // return bins after rebinning
  return evis_bins_rebin;
}

Int_t Predictor::GetNumRebinnedEvisBins()
{ // return bins after rebinning
...
void Predictor::LoadMainData(const Char_t* mainmatrixname)
{
  ...
  while (!mainfile.eof()) {
    ...
    if (linenum == 1) {
      if (atoi(firstchar.c_str()) == 0)
        isMC = true;
      if (atoi(firstchar.c_str()) == 1)
        isMC = false;
      cout << "Simflag: " << isMC << " (1-->MC, 0-->Data)" << endl;
    }
    // The meat of the file <----------------
    if (linenum > 2) {
      ; //<--the "meat" begins at line=3
      ...
      while (iss) {
        ...
        if (column == 1)
          row = atoi(sub.c_str());
        if (column > 1 && sub != "")
          readvals[column - 2] = atof(sub.c_str());
        column += 1;
      ...
      //-->bg events
      if (row == 9)
        continue; //<-- we don't care about total background

      //-->bg systematics
      if (row == 10)
        continue; //<-- we don't care about background systematics
      //-->acc bg
      if (row == 11) {
        for (int ii = 0; ii < Ndetectors; ii++) {
          tdper[istage - 1].AccEvts[ii] = readvals[ii];
          // in new input format have to correct for efficiencies here
          tdper[istage - 1].AccEvts[ii] *=
                           ...
                           const Char_t* muondecayspecname)
{
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 0; idet < Ndetectors; ++idet) {
      sprintf(name, "h_accidental_eprompt_inclusive_eh%i_ad%i",
              detConfigEH[idet], detConfigAD[idet]);
      ...
      for (Int_t ibin = 0;
           ibin < tdper[istage].CorrAccEvtsSpec[idet]->GetNbinsX(); ibin++) {
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 0; idet < Ndetectors; ++idet) {
      if (tdper[istage].CorrAccEvtsSpec[idet]->Integral() == 0)
        ...
        tdper[istage].CorrAccEvtsSpec[idet]->Scale(
            tdper[istage].AccEvts[idet] * 1. /
            tdper[istage]
                .CorrAccEvtsSpec[idet]
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 0; idet < Ndetectors; ++idet) {
      ...
      for (Int_t ibin = 0; ibin < htemp->GetNbinsX(); ibin++) {
        tdper[istage].CorrLi9EvtsSpec[idet]->Fill(
      ...
      for (Int_t ibin = 0;
           ibin < tdper[istage].CorrLi9EvtsSpec[idet]->GetNbinsX(); ibin++) {
        tdper[istage].CorrLi9EvtsSpec[idet]->SetBinError(ibin + 1, 0);
      }
      // cout << "The scaling factor is " << tdper[istage].Li9Evts[idet] << ","
      // << tdper[istage].CorrLi9EvtsSpec[idet]->Integral()  << endl;//tmp
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 0; idet < Ndetectors; ++idet) {
      ...
      // fill into destination histogram
      for (Int_t ibin = 0; ibin < htemp->GetNbinsX(); ibin++) {
        tdper[istage].CorrFnEvtsSpec[idet]->Fill(
            htemp->GetBinCenter(ibin + 1), htemp->GetBinContent(ibin + 1));
      }
      delete htemp;

      for (Int_t ibin = 0;
           ibin < tdper[istage].CorrFnEvtsSpec[idet]->GetNbinsX(); ibin++) {
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 0; idet < Ndetectors; ++idet) {
      ...
      for (int ibin = 1;
           ibin <= tdper[istage].CorrAmcEvtsSpec[idet]->GetXaxis()->GetNbins();
           ++ibin) {
        double lowedge =
            tdper[istage].CorrAmcEvtsSpec[idet]->GetXaxis()->GetBinLowEdge(
                ibin);
        double upedge =
            tdper[istage].CorrAmcEvtsSpec[idet]->GetXaxis()->GetBinUpEdge(ibin);
      ...
      // old
      // tdper[istage].CorrAmcEvtsSpec[idet] =
      // (TH1F*)m_amcspec->Get("h_rebin")->Clone(name);

      for (Int_t ibin = 0;
           ibin < tdper[istage].CorrAmcEvtsSpec[idet]->GetNbinsX(); ibin++) {
      ...
      if (tdper[istage].CorrAmcEvtsSpec[idet]->Integral() == 0)
        ...
        tdper[istage].CorrAmcEvtsSpec[idet]->Scale(
            tdper[istage].AmcEvts[idet] * 1. /
            tdper[istage]
                .CorrAmcEvtsSpec[idet]
                ->Integral()); //<---Scale this histogram to expected number of
                               // events per liveday based on input file
    }
  }
  // m_amcspec->Close();
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 0; idet < Ndetectors; ++idet) {
      ...
      // assign zero errors
      for (Int_t ibin = 0;
           ibin < tdper[istage].CorrAlnEvtsSpec[idet]->GetNbinsX(); ibin++) {
      ...
      if (tdper[istage].CorrAlnEvtsSpec[idet]->Integral() == 0)
        ...
        tdper[istage].CorrAlnEvtsSpec[idet]->Scale(
            tdper[istage].AlnEvts[idet] * 1. /
            tdper[istage]
                .CorrAlnEvtsSpec[idet]
                ->Integral()); //<---Scale this histogram to expected number of
                               // events based on input file
    }
  }
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 0; idet < Ndetectors; ++idet) {
      ...
      tdper[istage].CorrBgEvtsSpec[idet] =
          (TH1F*)tdper[istage].CorrAccEvtsSpec[idet]->Clone(name);
      tdper[istage].CorrBgEvtsSpec[idet]->Add(
          tdper[istage].CorrLi9EvtsSpec[idet], 1);
      tdper[istage].CorrBgEvtsSpec[idet]->Add(
          tdper[istage].CorrAmcEvtsSpec[idet], 1);
      tdper[istage].CorrBgEvtsSpec[idet]->Add(
...
TH1F* Predictor::GetCorrAccEvtsSpec(Int_t istage, Int_t idet)
{
  ...
  return dummy;
  // return
  // tdper[istage].CorrAccEvtsSpec[idet]->Scale(tdper[istage].Livetime[idet]);
}
TH1F* Predictor::GetCorrLi9EvtsSpec(Int_t istage, Int_t idet)
{
  ...
  return dummy;
  // return
  // tdper[istage].CorrLi9EvtsSpec[idet]->Scale(tdper[istage].Livetime[idet]);
}
TH1F* Predictor::GetCorrAmcEvtsSpec(Int_t istage, Int_t idet)
{
  ...
  return dummy;
  // return
  // tdper[istage].CorrAmcEvtsSpec[idet]->Scale(tdper[istage].Livetime[idet]);
}
TH1F* Predictor::GetCorrFnEvtsSpec(Int_t istage, Int_t idet)
{
  ...
  return dummy;
  // return
  // tdper[istage].CorrFnEvtsSpec[idet]->Scale(tdper[istage].Livetime[idet]);
}
...
void Predictor::EnterObsEvts(double obsad1, double obsad2, double obsad3,
                             double obsad4, double obsad5, double obsad6,
                             double obsad7, double obsad8, int istage)
{
...
TH1F* Predictor::GetCorrEvtsSpec(int istage, int idet)
{
  ...
  // dummy->Scale(tdper[istage].Livetime[idet]);// *
  // tdper[istage].MuonVetoEff[idet] * tdper[istage].DMCEff[idet]); return
  // dummy; return
  // tdper[istage].CorrEvtsSpec[idet]->Scale(tdper[istage].Livetime[idet]);
...
void Predictor::MakePrediction(double sin2theta13, double dm2, double s22t14,
                               double dm2_41, int istage,
                               TimePeriodData& tperdat)
{ //, PredSet *predout){
  for (int idet = 0; idet < 4; ++idet) {
    // Convert from Evis to Eneu_true
    // The procedure is rather complex now in oreder to rescale Evis-to-Enu
    // conversion matrix using the oscillation probability.
    std::map<int, TH1F*> mapflux_noosc = fluxcalc->CalculateFluxHistRow(
        idet, sin2theta13, istage, dm2, s22t14, dm2_41, 0);
    // Get the inputs to calculate reactor flux contribution from each core.
    // This information is needed in order to properly rescale the matrix using
    // oscillation probability. For this, we should not assume any oscillation,
    // because evis-to-enu matrix is generated without any oscillation. The
    // final argument of CalculateFluxHistRow, "0",  removes any oscillation
    // effects.
    if (tperdat.CorrEvtsSpec[idet]->GetXaxis()->GetNbins() != n_evis_bins) {
      cout << "Error Error Error! Inconsistent number of bins for the spectrum "
              "inputs "
           << tperdat.CorrEvtsSpec[idet]->GetXaxis()->GetNbins() << " vs. "
           << n_evis_bins << endl;
    }
    ...
    for (int ibin_enu = 0; ibin_enu < n_enu_bins; ++ibin_enu) {
      ...
      double Etrue =
          CorrEvtsTrueSpec[istage][idet][0]->GetXaxis()->GetBinCenter(ibin_enu +
                                                                      1);

      for (int icore = 0; icore < Ncores; ++icore) {
        int thebin = mapflux_noosc[icore]->GetXaxis()->FindBin(Etrue);
        factor_noosc[ibin_enu][icore] =
            mapflux_noosc[icore]->GetBinContent(thebin);
    ...
    std::map<int, TH1F*> mapextrap = fluxcalc->ExtrapolationFactorRow(
        idet, -1, sin2theta13, istage, dm2, s22t14, dm2_41);

    for (int ibin_evis = 0; ibin_evis < n_evis_bins; ++ibin_evis) {
      Double_t norm = 0;
      double oldval = tperdat.CorrEvtsSpec[idet]->GetBinContent(ibin_evis + 1);
      if (oldval < 0) {
        cout << "Predictor: Negative bin content after background subtraction "
                "for detector "
             << idet << ", bin " << ibin_evis << endl;
      ...
      for (int ibin_enu = 0; ibin_enu < n_enu_bins; ++ibin_enu) {
        for (int icore = 0; icore < Ncores; ++icore) {
          double Etrue = CorrEvtsTrueSpec[istage][idet][ibin_evis]
                             ->GetXaxis()
                             ->GetBinCenter(ibin_enu + 1);
          int thebin = mapflux_noosc[icore]->GetXaxis()->FindBin(Etrue);
          double oscprob = mapextrap[icore]->GetBinContent(thebin);
          CorrEvtsTrueSpec[istage][idet][ibin_evis]->AddBinContent(
              ibin_enu + 1, oldval * M_evis_to_enu[ibin_evis][ibin_enu] *
                                factor_noosc[ibin_enu][icore] * oscprob);

          norm += M_evis_to_enu[ibin_evis][ibin_enu] *
                  factor_noosc[ibin_enu][icore] * oscprob;
        }
      }
      if (norm != 0)
    ...
    std::map<int, TH1F*> mapflux = fluxcalc->CalculateFluxHistRow(
        idet, sin2theta13, istage, dm2, s22t14, dm2_41);
    for (int ibin_evis = 0; ibin_evis < n_evis_bins; ++ibin_evis) {
      for (int icore = 0; icore < Ncores; ++icore) {
        if (FirstMakePrediction) {
          // Need to ensure other istages are created
          for (int ii = 0; ii < Nstage; ++ii) {
            sprintf(dummyname, "CorrEvtsCoreSpec_%i_%i_%i_%i", ii, idet,
                    ibin_evis, icore);
            CorrEvtsCoreSpec[ii][idet][ibin_evis][icore] =
                (TH1F*)CorrEvtsTrueSpec[ii][idet][ibin_evis]->Clone(dummyname);
          }
        }
        CorrEvtsCoreSpec[istage][idet][ibin_evis][icore]->Reset();
        // Note: multiplication has to be done manually in order to allow
        // different bins between E hists and flux hists
        for (int ibin = 1;
             ibin <=
             CorrEvtsTrueSpec[istage][idet][ibin_evis]->GetXaxis()->GetNbins();
             ++ibin) {
          double Etrue = CorrEvtsTrueSpec[istage][idet][ibin_evis]
                             ->GetXaxis()
                             ->GetBinCenter(ibin);
          //          if (Etrue > 8.7) Etrue = 8.7; // this is because the
          //          current flux histogram ends at 8.8 MeV
          int thebin = mapflux[icore]->GetXaxis()->FindBin(Etrue);
          double factor = mapflux[icore]->GetBinContent(thebin);
          CorrEvtsCoreSpec[istage][idet][ibin_evis][icore]->SetBinContent(
              ibin,
              CorrEvtsTrueSpec[istage][idet][ibin_evis]->GetBinContent(ibin) *
                  factor);
        }
      }
    }
  ...
  for (int idet = 4; idet < 8; ++idet) {
    for (int idet2 = 0; idet2 < 4; ++idet2) {
      std::map<int, TH1F*> mapextrap = fluxcalc->ExtrapolationFactorRow(
          idet, idet2, sin2theta13, istage, dm2, s22t14, dm2_41);
      for (int ibin_evis = 0; ibin_evis < n_evis_bins; ++ibin_evis) {
        if (FirstMakePrediction) {
          // Need to ensure other istages are created
          for (int ii = 0; ii < Nstage; ++ii) {
            sprintf(dummyname, "PredEvtsTrueSpec_%i_%i_%i_%i", ii, idet, idet2,
                    ibin_evis);
            PredEvtsTrueSpec[ii][idet][idet2][ibin_evis] =
                (TH1F*)CorrEvtsCoreSpec[ii][idet2][ibin_evis][0]->Clone(
                    dummyname);
          }
        }
        ...
        for (int icore = 0; icore < Ncores; ++icore) {
          // same as above: have to do multiplication manually to allow for
          // different bins
          for (int ibin = 1;
               ibin <= CorrEvtsCoreSpec[istage][idet2][ibin_evis][icore]
                           ->GetXaxis()
                           ->GetNbins();
               ++ibin) {
            double Etrue = CorrEvtsCoreSpec[istage][idet2][ibin_evis][icore]
                               ->GetXaxis()
                               ->GetBinCenter(ibin);
            //            if (Etrue > 8.7) Etrue = 8.7; // this is because the
            //            current flux histogram ends at 8.8 MeV
            ...
            PredEvtsTrueSpec[istage][idet][idet2][ibin_evis]->AddBinContent(
                ibin, CorrEvtsCoreSpec[istage][idet2][ibin_evis][icore]
                              ->GetBinContent(ibin) *
                          factor);
          }
    ...
    for (int idet2 = 0; idet2 < 4; ++idet2) {
      ...
      if (NdetectorsConfig[istage][idet] != 0)
        if (NdetectorsConfig[istage][idet2] != 0)
          for (int ibin_evis = 0; ibin_evis < n_evis_bins; ibin_evis++)
            PredEvtsSpec[istage][idet][idet2]->SetBinContent(
                ibin_evis + 1,
                PredEvtsTrueSpec[istage][idet][idet2][ibin_evis]->Integral());
...
void Predictor::CombineData()
{
  for (int idet = 0; idet < Ndetectors; ++idet) {
    ...
    if (FirstMakeSuperPrediction) {
      ...
      CombCorrEvtsSpec[idet] =
          (TH1F*)tdper[0].CorrEvtsSpec[idet]->Clone(dummyname);
      sprintf(dummyname, "CombCorrBgEvtsSpec_AD%i", idet);
      CombCorrBgEvtsSpec[idet] =
          (TH1F*)tdper[0].CorrBgEvtsSpec[idet]->Clone(dummyname);
      // note: do not set FirstMakeSuperPrediction to false as that is done
      // below
    }
...
void Predictor::CombinePredictions()
{
  for (int idet = 4; idet < 8; ++idet) {
    for (int idet2 = 0; idet2 < 4; ++idet2) {
      ...
      if (FirstMakeSuperPrediction) {
        hweightedsum =
            (TH1F*)predper->GetPred(0, idet, idet2)->Clone(dummyname);
...
PredSet* Predictor::MakeOneSuperPrediction(double sin2theta13, double dm2,
                                           ...
                                           bool print)
{
  ...
  if (print) {
    ...
    cout << "================================================================="
         << endl;
    cout << "================= Super-prediction =============================="
         << endl;
    superpred->PrintToScreen(0);
    cout << "================= Combined Data ================================="
         << endl;
    for (int idet = 0; idet < Ndetectors; ++idet) {
      cout << "AD" << idet + 1 << ": " << CombCorrEvtsSpec[idet]->Integral()
           << endl; // tmp
    }
  }
...
void Predictor::MakeAllPredictions(double sin2theta13, double dm2,
                                   double s22t14, double dm2_41, bool print)
{
  // print = true;
  if (print) {
    cout << "-----------------------------------------------------------"
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    MakePrediction(sin2theta13, dm2, s22t14, dm2_41, istage,
                   tdper[istage]); //,predper[istage]);
...
void Predictor::LoadEvisToEnuMatrix(const Char_t* evis_to_enu_matrix_name)
{
  ...
  if (!matrix_file.is_open()) {
    cout << "Error: cannot find evis to enu convertion matrix "
         << evis_to_enu_matrix_name << endl;
    return;
  }
                              ...
                              const Char_t* covmatrixname_dm2ee)
{
  ...
  string thead;
  //-->Distances
  cout << " Signal coavariance matrix ++++++++++++++++++++++++++++++++++++++"
       << endl;
        ...
        for (int idet2 = 0; idet2 < 4; ++idet2) {
          for (int jdet = 4; jdet < 8; ++jdet) {
            for (int jdet2 = 0; jdet2 < 4; ++jdet2) {
              ...
              if (NdetectorsConfig[jstage][jdet] == 0)
                continue;
              if (NdetectorsConfig[jstage][jdet2] == 0)
                continue;
              ...
              for (Int_t i = 0; i < n_evis_bins; i++) {
                for (Int_t j = 0; j < n_evis_bins; j++) {
                  ...
                  M_bg_sys_frac[i2][j2] = M_bg_sys[i2][j2] /
                                          predper->GetPred(istage, idet, idet2)
                                              ->GetBinContent(i + 1) /
                                          predper->GetPred(jstage, jdet, jdet2)
                                              ->GetBinContent(j + 1);

                  if (type == -1 || type == 0) { // all errors
                    M[i2][j2] += M_sig_sys[i2][j2] *
                                     predper->GetPred(istage, idet, idet2)
                                         ->GetBinContent(i + 1) *
                                     predper->GetPred(jstage, jdet, jdet2)
                                         ->GetBinContent(j + 1) +
                    ...
                    /*
                      if(n_evis_bins==37){ //ensure to do this only for LBNL
                      binning if(i==j){ //only for diagonal elements if(i<3){
                      //only for first three bins
                      ...
                      * predper->GetPred(jstage,jdet,jdet2)->GetBinContent(j+1);
                      //adding 10%, 20%, 40% additional uncertainty
                      }
                      }
                      }*/
...
void Predictor::InvertMatrix(int step_sin22t13)
{
  ...
  if (stage == -1) { // Use full covariance matrix
    ...
    TMatrixD* mat =
        new TMatrixD(npredictions * Stages_in_68AD * n_evis_bins_rebin,
                     npredictions * Stages_in_68AD * n_evis_bins_rebin);

    Double_t M_scaled_tmp_stage[npredictions * Stages_in_68AD *
                                n_evis_bins_rebin * npredictions *
                                Stages_in_68AD * n_evis_bins_rebin];


    for (int i = 0; i < npredictions * Stages_in_68AD * n_evis_bins_rebin;
         ++i) {
      for (int j = 0; j < npredictions * Stages_in_68AD * n_evis_bins_rebin;
           ++j) {
        M_scaled_tmp_stage[i * Stages_in_68AD * npredictions *
                               n_evis_bins_rebin +
                           j] =
            M_scaled_tmp[i * npredictions * Nstage * n_evis_bins_rebin + j];
    ...
    delete mat;
  } else { // Need to be very careful on how to get the correct portion of the
           // covmatrix
    TMatrixD* mat = new TMatrixD(npredictions * n_evis_bins_rebin,
                                 npredictions * n_evis_bins_rebin);

    Double_t M_scaled_tmp_stage[npredictions * n_evis_bins_rebin *
                                npredictions * n_evis_bins_rebin];

    for (int i = 0; i < npredictions * n_evis_bins_rebin; ++i) {
      for (int j = 0; j < npredictions * n_evis_bins_rebin; ++j) {
        M_scaled_tmp_stage[i * npredictions * n_evis_bins_rebin + j] =
            M_scaled_tmp[(stage * npredictions * n_evis_bins_rebin + i) *
                             npredictions * Nstage * n_evis_bins_rebin +
                         (stage * npredictions * n_evis_bins_rebin + j)];
        // mat(i,j) =
        // M_scaled_tmp[(stage*npredictions*n_evis_bins_rebin+i)*npredictions*Nstage*n_evis_bins_rebin+(stage*npredictions*n_evis_bins_rebin+j)];
      }
    }
...
void Predictor::InvertRateMatrix()
{
              ...
              for (int jdet2 = 0; jdet2 < 4; ++jdet2) {
                ...
                for (int ie = 0; ie < n_evis_bins; ++ie) {
                  for (int je = 0; je < n_evis_bins; ++je) {
                    if (stage == -1 ||
                        stage == 7) // I guess this is proper extension
                      M_scaled[nNearHalls * istage + (detConfigEH[idet2] - 1)]
                              [nNearHalls * jstage +
                               (detConfigEH[jdet2] - 1)] +=
                          M[(istage * MaxPredictions + iii) * n_evis_bins + ie]
                           [(jstage * MaxPredictions + jjj) * n_evis_bins + je];
                    else
                      M_scaled[(detConfigEH[idet2] - 1)]
                              [(detConfigEH[jdet2] - 1)] +=
                          M[(istage * MaxPredictions + iii) * n_evis_bins + ie]
                           [(jstage * MaxPredictions + jjj) * n_evis_bins + je];
                  }
      ...
      for (Int_t jstage = 0; jstage < Nstage; jstage++) {
        for (Int_t i = 0; i < nNearHalls; i++) {
          for (Int_t j = 0; j < nNearHalls; j++) {
            if (stage == -1)
              M_rate_inv[istage * nNearHalls + i][jstage * nNearHalls + j] =
                  m_tmp[((istage * nNearHalls) + i) * nNearHalls * Nstage +
                        (jstage * nNearHalls) + j];
            else if (stage == 7) {
  ...
  if (combine_mode == 0) {
    if (n_evis_bins_rebin != n_evis_bins)
      cout << "Bin mismatch for some reason" << endl;
          ...
          for (int idet2 = 0; idet2 < 4; ++idet2) {
            ...
            for (int jdet = 4; jdet < 8; ++jdet) {
              ...
              for (int jdet2 = 0; jdet2 < 4; ++jdet2) {
                ...
                for (int ie = 0; ie < n_evis_bins; ++ie) {
                  for (int je = 0; je < n_evis_bins; ++je) {
                    M_scaled[npredictions * istage + iii]
                            [npredictions * jstage + jjj] +=
                        M[(istage * MaxPredictions + iii) * n_evis_bins + ie]
                         [(jstage * MaxPredictions + jjj) * n_evis_bins + je];
                  }
                }
      ...
      for (Int_t jstage = 0; jstage < Nstage; jstage++) {
        for (Int_t i = 0; i < npredictions; i++) {
          for (Int_t j = 0; j < npredictions; j++) {
            M_scaled_tmp[(istage * npredictions + i) * npredictions * Nstage +
                         jstage * npredictions + j] =
                M_scaled[istage * npredictions + i][jstage * npredictions + j];
          }
        }
      }
    }
    ...
    for (Int_t istage = 0; istage < Nstage; istage++) {
      for (Int_t jstage = 0; jstage < Nstage; jstage++) {
        for (Int_t i = 0; i < npredictions; i++) {
          for (Int_t j = 0; j < npredictions; j++) {
            M_rate_inv[istage * npredictions + i][jstage * npredictions + j] =
                m_tmp[((istage * npredictions) + i) * npredictions * Nstage +
                      (jstage * npredictions) + j];
...
Double_t Predictor::CalculateChi2Cov(Double_t sin22t13)
{
    ...
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        for (Int_t i = 0; i < n_evis_bins; i++) {
          if (combine_mode == 0) {
            Int_t iii = (idet - 4) * 4 + idet2;
            N_obs[(npredictions * istage + iii) * n_evis_bins_rebin +
                  evis_rebin_map[i]] +=
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
            N_pred[(npredictions * istage + iii) * n_evis_bins_rebin +
                   evis_rebin_map[i]] +=
                predper->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          }

          if (combine_mode == 1) {
            N_obs[(npredictions * istage + (detConfigEH[idet2] - 1)) *
                      n_evis_bins_rebin +
                  evis_rebin_map[i]] +=
                mode1_coeff[istage][idet2] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
            N_pred[(npredictions * istage + (detConfigEH[idet2] - 1)) *
                       n_evis_bins_rebin +
                   evis_rebin_map[i]] +=
                mode1_coeff[istage][idet2] *
                predper->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          ...
          if (combine_mode == 2) {
            N_obs[(npredictions * istage + idet2) * n_evis_bins_rebin +
                  evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
            N_pred[(npredictions * istage + idet2) * n_evis_bins_rebin +
                   evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                predper->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          }
          ...
          if (combine_mode == 3) {
            N_obs[npredictions * istage * n_evis_bins_rebin +
                  evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
            N_pred[npredictions * istage * n_evis_bins_rebin +
                   evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                predper->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          }
  ...
  Double_t chi2out = 0;
  for (Int_t i = npredictions * startStage * n_evis_bins_rebin;
       i < npredictions * endStage * n_evis_bins_rebin; i++) {
    // cout << "i: " << i << " N_obs: " << N_obs[i]<< " N_pred: " << N_pred[i]
    // << endl;
    for (Int_t j = npredictions * startStage * n_evis_bins_rebin;
         j < npredictions * endStage * n_evis_bins_rebin; j++) {
      // cout << "j: " << j << " N_obs: " << N_obs[j]<< " N_pred: " << N_pred[j]
      // << endl;

      // cout << "M_inv: " << M_inv[i][j] << " chi2: " <<
      // M_inv[i][j]*(N_obs[i]-N_pred[i])*(N_obs[j]-N_pred[j]) << endl; for
      // (Int_t i = 0; i < npredictions*Nstage*n_evis_bins_rebin; i++){ for
      // (Int_t j = 0; j < npredictions*Nstage*n_evis_bins_rebin; j++){
      chi2out += M_inv[i][j] * (N_obs[i] - N_pred[i]) * (N_obs[j] - N_pred[j]);
    }
  }
...
Double_t Predictor::CalculateChi2CovRate(Double_t sin22t13, Double_t dm2_ee,
                                         Double_t sin22t14, Double_t dm2_41)
{
  this->MakeAllPredictions(sin22t13, dm2_ee, sin22t14, dm2_41, false);
    ...
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        if (combine_mode == 0) {
          Int_t iii = (idet - 4) * 4 + idet2;
          for (Int_t i = 0; i < n_evis_bins; i++) {
            N_obs[istage * npredictions + iii] +=
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
            N_pred[istage * npredictions + iii] +=
                predper->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          }
        }
        if (combine_mode == 1) {
          for (Int_t i = 0; i < n_evis_bins; i++) {
            N_obs[istage * npredictions + (detConfigEH[idet2] - 1)] +=
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
            N_pred[istage * nNearHalls + (detConfigEH[idet2] - 1)] +=
                predper->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          }
        }
      }
    ...
    for (Int_t jstage = 0; jstage < Nstage; jstage++) {
      for (Int_t i = 0; i < npredictions; i++) {
        for (Int_t j = 0; j < npredictions; j++) {
          chi2out +=
              M_rate_inv[istage * npredictions + i][jstage * npredictions + j] *
              (N_obs[istage * npredictions + i] -
               N_pred[istage * npredictions + i]) *
              (N_obs[jstage * npredictions + j] -
               N_pred[jstage * npredictions + j]);
        }
      }
    }
  }
  // cout << chi2out << " " << N_obs[0]  << " " << N_obs[1]  << " " << N_pred[0]
  // << " " << N_pred[1] << endl;
                             ...
                             Double_t sin22t14, Double_t dm2_41)
{
  ...
  cout << "Calculated and fixed the covariance matrix using parameters: "
       << sin22t13 << " " << dm2_ee << " " << sin22t14 << " " << dm2_41
       << " Stage: " << stage << endl;
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        for (Int_t i = 0; i < n_evis_bins; i++) {
          if (mode == 0)
            final_pred[(npredictions * istage + iii) * n_evis_bins_rebin +
                       evis_rebin_map[i]] +=
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          if (mode == 1)
            final_pred[(npredictions * istage + (detConfigEH[idet2] - 1)) *
                           n_evis_bins_rebin +
                       evis_rebin_map[i]] +=
                mode1_coeff[istage][idet2] *
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          if (mode == 2)
            final_pred[(npredictions * istage + idet2) * n_evis_bins_rebin +
                       evis_rebin_map[i]] +=
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          if (mode == 3)
            final_pred[npredictions * istage * n_evis_bins_rebin +
                       evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
        }
  ...
  for (int istage = 0; istage < Nstage; istage++) {
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        for (Int_t i = 0; i < n_evis_bins; i++) {
          if (mode == 0)
            final_obs[(istage * npredictions + iii) * n_evis_bins_rebin +
                      evis_rebin_map[i]] +=
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
          if (mode == 1)
            final_obs[(istage * npredictions + detConfigEH[idet2] - 1) *
                          n_evis_bins_rebin +
                      evis_rebin_map[i]] +=
                mode1_coeff[istage][idet2] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
          if (mode == 2)
            final_obs[(istage * npredictions + idet2) * n_evis_bins_rebin +
                      evis_rebin_map[i]] +=
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
          if (mode == 3)
            final_obs[istage * npredictions * n_evis_bins_rebin +
                      evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
        }
  ...
  for (int istage = 0; istage < Nstage; istage++) {
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        for (Int_t i = 0; i < n_evis_bins; i++) {
          if (mode == 0)
            final_obserror[(istage * npredictions + iii) * n_evis_bins_rebin +
                           evis_rebin_map[i]] +=
                pow(tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1), 2);
          if (mode == 1)
            final_obserror[(istage * npredictions + detConfigEH[idet2] - 1) *
                               n_evis_bins_rebin +
                           evis_rebin_map[i]] +=
                mode1_coeff[istage][idet2] *
                pow(tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1), 2);
          if (mode == 2)
            final_obserror[(istage * npredictions + idet2) * n_evis_bins_rebin +
                           evis_rebin_map[i]] +=
                pow(tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1), 2);
          if (mode == 3)
            final_obserror[istage * npredictions * n_evis_bins_rebin +
                           evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                pow(tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1), 2);
        }
  ...
  for (int istage = 0; istage < Nstage; ++istage) {
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        for (Int_t i = 0; i < n_evis_bins; i++) {
          if (mode == 0) {
            final_pred_sum[iii * n_evis_bins_rebin + evis_rebin_map[i]] +=
                FarAD_stage_coeff[istage][idet] *
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          }
          if (mode == 1)
            final_pred_sum[(detConfigEH[idet2] - 1) * n_evis_bins_rebin +
                           evis_rebin_map[i]] +=
                FarEH_stage_coeff[istage][idet] * mode1_coeff[istage][idet2] *
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          if (mode == 2)
            final_pred_sum[idet2 * n_evis_bins_rebin + evis_rebin_map[i]] +=
                FarAD_stage_coeff[istage][idet] *
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
          if (mode == 3)
            final_pred_sum[evis_rebin_map[i]] +=
                FarEH_stage_coeff[istage][idet] * NearEH_coeff[istage][idet2] *
                evtset->GetPred(istage, idet, idet2)->GetBinContent(i + 1);
        }
      }
    }
  ...
  for (int istage = 0; istage < Nstage; istage++) {
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        for (Int_t i = 0; i < n_evis_bins; i++) {
          if (mode == 0)
            final_obs_sum[iii * n_evis_bins_rebin + evis_rebin_map[i]] +=
                FarAD_stage_coeff[istage][idet] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
          if (mode == 1)
            final_obs_sum[(detConfigEH[idet2] - 1) * n_evis_bins_rebin +
                          evis_rebin_map[i]] +=
                FarEH_stage_coeff[istage][idet] * mode1_coeff[istage][idet2] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
          if (mode == 2)
            final_obs_sum[idet2 * n_evis_bins_rebin + evis_rebin_map[i]] +=
                FarAD_stage_coeff[istage][idet] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
          if (mode == 3)
            final_obs_sum[evis_rebin_map[i]] +=
                FarEH_stage_coeff[istage][idet] * NearEH_coeff[istage][idet2] *
                tdper[istage].CorrEvtsSpec[idet]->GetBinContent(i + 1);
        }
      }
  ...
  for (int istage = 0; istage < Nstage; istage++) {
    for (int idet = 4; idet < 8; ++idet) {
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        ...
        for (Int_t i = 0; i < n_evis_bins; i++) {
          if (mode == 0)
            final_obserror_sum[iii * n_evis_bins_rebin + evis_rebin_map[i]] +=
                pow(FarAD_stage_coeff[istage][idet] *
                        tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1),
                    2);
          if (mode == 1)
            final_obserror_sum[(detConfigEH[idet2] - 1) * n_evis_bins_rebin +
                               evis_rebin_map[i]] +=
                mode1_coeff[istage][idet2] *
                pow(FarEH_stage_coeff[istage][idet] *
                        tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1),
                    2);
          if (mode == 2)
            final_obserror_sum[idet2 * n_evis_bins_rebin + evis_rebin_map[i]] +=
                pow(FarAD_stage_coeff[istage][idet] *
                        tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1),
                    2);
          if (mode == 3)
            final_obserror_sum[evis_rebin_map[i]] +=
                NearEH_coeff[istage][idet2] *
                pow(FarEH_stage_coeff[istage][idet] *
                        tdper[istage].CorrEvtsSpec[idet]->GetBinError(i + 1),
                    2);
      ...
      for (int idet2 = 0; idet2 < 4; ++idet2) {
        for (int jdet = 4; jdet < 8; ++jdet) {
          for (int jdet2 = 0; jdet2 < 4; ++jdet2) {
            ...
            if (idet2 ==
                jdet2) { // sharing the same near detector during the same stage
              // cout << "sharing near detector " << iii << " " << jjj << endl;
              for (Int_t i = 0; i < n_evis_bins; i++) {
                Int_t j = i; // Only i==j matters
                // Scale near site statistical error ( = sqrt(N_tot^2 + N_bg^2))
                // to the far site "evtset-> GetEvts(idet,idet2) /
                // CorrEvts[idet2]" gives the far/near ratio at given theta 13
                // value

                Double_t err_i = 0;
                if (tdper[istage].CorrEvtsSpec[idet2]->GetBinContent(i + 1) > 0)
                  err_i =
                      tdper[istage].CorrEvtsSpec[idet2]->GetBinError(i + 1) *
                      predper->GetPred(istage, idet, idet2)
                          ->GetBinContent(i + 1) /
                      tdper[istage].CorrEvtsSpec[idet2]->GetBinContent(i + 1);
                Double_t err_j = 0;
                if (tdper[jstage].CorrEvtsSpec[jdet2]->GetBinContent(j + 1) > 0)
                  err_j =
                      tdper[jstage].CorrEvtsSpec[jdet2]->GetBinError(j + 1) *
                      predper->GetPred(jstage, jdet, jdet2)
                          ->GetBinContent(j + 1) /
                      tdper[jstage].CorrEvtsSpec[jdet2]->GetBinContent(j + 1);


                M_stat[(MaxPredictions * istage + iii) * n_evis_bins + i]
                      [(MaxPredictions * jstage + jjj) * n_evis_bins + j] +=
                    err_i * err_j / stat_factor[istage];

                M_stat_frac[(MaxPredictions * istage + iii) * n_evis_bins + i]
                           [(MaxPredictions * jstage + jjj) * n_evis_bins +
                            j] +=
                    err_i * err_j /
                    predper->GetPred(istage, idet, idet2)
                        ->GetBinContent(i + 1) /
                ...
                // cout << "M_stat: " <<
                // M_stat[(MaxPredictions*istage+iii)*n_evis_bins+i][(MaxPredictions*jstage+jjj)*n_evis_bins+j]
                // << endl;
              }
            }
            ...
            if (idet ==
                jdet) { // sharing the same far detector during the same stage
              ...
              for (Int_t i = 0; i < n_evis_bins; i++) {
                ...
                if (factor > 0) {
                  ...
                  if (predper->GetPred(istage, idet, idet2)
                              ->GetBinContent(i + 1) != 0 &&
                      predper->GetPred(jstage, jdet, jdet2)
                              ->GetBinContent(j + 1) != 0)
                    M_stat_frac[(MaxPredictions * istage + iii) * n_evis_bins +
                                i]
                               [(MaxPredictions * jstage + jjj) * n_evis_bins +
                                j] += err_i * err_j /
                                      predper->GetPred(istage, idet, idet2)
                ...
                // to cross-check
                // Double_t err_i =
                // tperdata->CorrEvtsSpec[idet]->GetBinError(i+1); Double_t
                // err_j = tperdata->CorrEvtsSpec[jdet]->GetBinError(j+1);
...
Double_t* Predictor::GetFinalCovMatrix(Int_t type, Int_t mode)
{
  if (RecalculateCovMatrix) {
    AddandScaleCovMatrix(
        type); // make matrix with a subset of error for display purpose
  }
...
Double_t* Predictor::CombineMatrix(Int_t mode, Bool_t MakeSum)
{
  ...
  if (CalculateMode1Coeff == true) {
    for (Int_t istage = 0; istage < Nstage; istage++) {
      mode1_coeff[istage][0] =
          (double)NdetectorsConfig[istage][0] /
          (double)(NdetectorsConfig[istage][0] + NdetectorsConfig[istage][1]);
      mode1_coeff[istage][1] =
          (double)NdetectorsConfig[istage][1] /
          (double)(NdetectorsConfig[istage][0] + NdetectorsConfig[istage][1]);
      mode1_coeff[istage][2] =
          (double)NdetectorsConfig[istage][2] /
          (double)(NdetectorsConfig[istage][2] + NdetectorsConfig[istage][3]);
      mode1_coeff[istage][3] =
          (double)NdetectorsConfig[istage][3] /
          (double)(NdetectorsConfig[istage][2] + NdetectorsConfig[istage][3]);
    }
  ...
  if (CalculateStageCoeff == true) {
    ...
    Double_t
        FarEH_IBD[Nstage]; // Keep track of total EH3 IBD for a single period
    for (Int_t istage = 0; istage < Nstage; istage++)
      FarEH_IBD[istage] = 0;
    ...
    for (Int_t idet = 4; idet < Ndetectors; ++idet) {
      ...
      for (Int_t istage = 0; istage < Nstage; istage++) {
        ...
        cout << "idet: " << idet << ", istage: " << istage
             << ", nIBD: " << nIBD[istage][idet]
             << ", FarAD: " << FarAD_stage_coeff[istage][idet] << endl;
      }
    }

    // Now that we have total EH3 IBD for all period, can calculate EH3 period
    // weighting
    for (Int_t istage = 0; istage < Nstage; istage++) {
      for (Int_t idet = 4; idet < Ndetectors; ++idet) {
        FarEH_stage_coeff[istage][idet] =
            FarEH_IBD[istage] / TotalFarEH_IBD; // No dependence on det

        cout << "istage: " << istage << ", idet: " << idet
             << ", nIBD: " << nIBD[istage][idet]
             << ", FarEH: " << FarEH_stage_coeff[istage][idet] << endl;
      }
    }
                ...
                for (int je = 0; je < n_evis_bins; ++je) {
                  if (MakeSum) {
                    if (mode == 0) { // no combining
                      M_scaled[(iii)*n_evis_bins_rebin + evis_rebin_map[ie]]
                              [(jjj)*n_evis_bins_rebin + evis_rebin_map[je]] +=
                          FarAD_stage_coeff[istage][idet] *
                          FarAD_stage_coeff[jstage][jdet] *
                          M[(MaxPredictions * istage + iii) * n_evis_bins + ie]
                           [(MaxPredictions * jstage + jjj) * n_evis_bins + je];
                    }
                    if (mode == 1) { // Combine to 2x2 matrix
                      M_scaled[(detConfigEH[idet2] - 1) * n_evis_bins_rebin +
                               evis_rebin_map[ie]]
                              [(detConfigEH[jdet2] - 1) * n_evis_bins_rebin +
                               evis_rebin_map[je]] +=
                          FarEH_stage_coeff[istage][idet] *
                          FarEH_stage_coeff[jstage][jdet] *
                          mode1_coeff[istage][idet2] *
                          mode1_coeff[jstage][jdet2] *
                          M[(MaxPredictions * istage + iii) * n_evis_bins + ie]
                           [(MaxPredictions * jstage + jjj) * n_evis_bins + je];
                    }
                    if (mode == 2) { // Combine to 4x4 matrix
                      M_scaled[(idet2)*n_evis_bins_rebin + evis_rebin_map[ie]]
                              [(jdet2)*n_evis_bins_rebin +
                               evis_rebin_map[je]] +=
                          FarAD_stage_coeff[istage][idet] *
                          FarAD_stage_coeff[jstage][jdet] *
                          M[(MaxPredictions * istage + iii) * n_evis_bins + ie]
                    ...
                    if (mode == 3) {
                      M_scaled[evis_rebin_map[ie]][evis_rebin_map[je]] +=
                          FarEH_stage_coeff[istage][idet] *
                          FarEH_stage_coeff[jstage][jdet] *
                          NearEH_coeff[istage][idet2] *
                          NearEH_coeff[jstage][jdet2] *
                          M[(MaxPredictions * istage + iii) * n_evis_bins + ie]
                           [(MaxPredictions * jstage + jjj) * n_evis_bins + je];
                    }
                  }
                    ...
                    if (mode == 1) { // Combine to 2x2 matrix
                      M_scaled[(npredictions * istage +
                                (detConfigEH[idet2] - 1)) *
                                   n_evis_bins_rebin +
                               evis_rebin_map[ie]][(npredictions * jstage +
                                                    (detConfigEH[jdet2] - 1)) *
                                                       n_evis_bins_rebin +
                                                   evis_rebin_map[je]] +=
                          mode1_coeff[istage][idet2] *
                    ...
                    if (mode == 2) { // Combine to 4x4 matrix
                      M_scaled[(npredictions * istage + idet2) *
                                   n_evis_bins_rebin +
                               evis_rebin_map[ie]]
                              [(npredictions * jstage + jdet2) *
                                   n_evis_bins_rebin +
                               evis_rebin_map[je]] +=
                          M[(MaxPredictions * istage + iii) * n_evis_bins + ie]
                           [(MaxPredictions * jstage + jjj) * n_evis_bins + je];
                    ...
                    if (mode == 3) { // Combine to 1x1 matrix
                      M_scaled[(npredictions * istage) * n_evis_bins_rebin +
                               evis_rebin_map[ie]]
                              [(npredictions * jstage) * n_evis_bins_rebin +
                               evis_rebin_map[je]] +=
                          NearEH_coeff[istage][idet2] *
                          NearEH_coeff[jstage][jdet2] *
                          M[(MaxPredictions * istage + iii) * n_evis_bins + ie]
  ...
  if (MakeSum) {
    for (Int_t i = 0; i < npredictions * n_evis_bins_rebin; i++) {
      for (Int_t j = 0; j < npredictions * n_evis_bins_rebin; j++) {
        final_covmatrix[i * npredictions * n_evis_bins_rebin + j] =
            M_scaled[i][j];
      }
    }
  } else {
    for (Int_t i = 0; i < npredictions * Nstage * n_evis_bins_rebin; i++) {
      for (Int_t j = 0; j < npredictions * Nstage * n_evis_bins_rebin; j++) {
        final_covmatrix[i * npredictions * Nstage * n_evis_bins_rebin + j] =
            M_scaled[i][j];
      }
    }
  }

...
FluxCalculator::FluxCalculator(const Char_t *distancesfile, const Char_t *superhistname){
  ...
  cout << "----------- Creating flux calculator ---------------" << endl;
  cout << "Note: using super-histogram mode of flux-calculator, so need inclusive mode" << endl;
  LoadDistances(distancesfile);
  cout << " --> Distances loaded successfully" << endl;
  osccalc = new OscCalc();
  cout << " --> OscCalc created successfully" << endl;
...
#pragma omp single copyprivate(WeeklyFluxData)
  ...
  cout << "----------------- WARNING: using the same flux shape for all cores for testing -------------------------" << endl;

}//end of LoadWeeklyFlux

std::map<int,TH1F*> FluxCalculator::CalculateFluxHistRow(int idet, double s22t13, int istage, double dm2, double s22t14, double dm2_41, int term){

  //Sanity check
  //if(useSuperHists && iweek>0){
  //  cout << "Ay caramba!!! You are using super-hist mode but looping over an iweek larger than 0!! Cannot be. I recommend you abort!" << endl;
  //}
  ...
  for(int icore=0;icore<Ncores;++icore){
    ...
    //    hout[icore] = (TH1F*)OscSpecQuick(idet,icore,s22t13,iweek)->Clone("clone");
    //    OscSpecQuick(idet,icore,s22t13,iweek,dm2,hout[icore]); // not using for now, since it dosent speed up for one fit

    if(!useSuperHists){
      osccalc->OscSpec(Distance[idet][icore],s22t13,h_flux[istage][icore],hout[icore]);
    }
    else{
      osccalc->OscSpecBinInt(Distance[idet][icore],s22t13,h_super[istage][idet][icore],hout[icore],term);
    }

    //SuperMatrix now obsolete and replaced by super-histograms, which are a more correct treatment
    //if (useSuperMatrix){
  ...
  for(int icore=0;icore<Ncores;++icore){
    ...
    // if term is specified, do not divide by the total, so that one can compare with oter terms and detecgtors
...
double FluxCalculator::GetDistance(int idet, int icore){
  ...
  if(idet >= Ndetectors){
    cout << "WARNING!!! idet=" << idet << " larger than current number of detectors" << endl;
    return -1;
  }
  if(icore >= Ncores){
    cout << "WARNING!!! icore=" << icore << " larger than current number of detectors" << endl;
    return -1;
  }
...
std::map<int,TH1F*> FluxCalculator::ExtrapolationFactorRow(int idet_far, int idet_near, double s22t13, int istage, double dm2, double s22t14, double dm2_41){
  ...
  for(int icore=0;icore<Ncores;++icore){
    ...
    if(!useSuperHists){
      osccalc->OscSpec(Distance[idet_far][icore],s22t13, h_flux[istage][icore],hout[icore]);
    ...
    if (idet_near < 0){
      ...
      hout[icore]->Scale(pow(Distance[idet_near][icore],2)*1./pow(Distance[idet_far][icore],2));
    }
    mapout[icore]=hout[icore];
  }
  return mapout;
...
TH1F *FluxCalculator::OscSpecQuick(int idet, int icore, double s22t13, int iweek, double dm2){
...
void FluxCalculator::OscSpecQuick(int idet, int icore, double s22t13, int iweek, double dm2, TH1F* h_osc){

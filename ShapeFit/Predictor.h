...
public:
  ...
  void
  LoadMainData(const Char_t* mainmatrixname); //<-- if call this have to call
                                              //LoadBgSpec as well as that is
  ...
  void MakePrediction(double sin2theta13, double dm2, Double_t sin22t14,
                      Double_t dm2_41, int tperiod,
                      TimePeriodData& tperdat); //, PredSet *predout);//<--use
                                                // dm2=-1 to get default value
  void MakeAllPredictions(double sin2theta13, double dm2 = -1,
                          Double_t sin22t14 = 0, Double_t dm2_41 = -1,
                          bool print = false);
  PredSet* MakeOneSuperPrediction(double sin2theta13, double dm2 = -1,
                                  Double_t sin22t14 = 0, Double_t dm2_41 = -1,
                                  bool print = false);
  ...
  void EnterObsEvts(double obsad1, double obsad2, double obsad3, double obsad4,
                    double obsad5, double obsad6, double obsad7, double obsad8,
                    int istage);
  ...
  void SetStatFactor(
      Double_t fac); // set factor to artificailly increase statitstics, by
                     // reducing stat error by 1/sqrt(stat_factor)
  ...
  void FixCovMatrix(Double_t sin22t13, Double_t dm2_ee, Double_t sin22t14,
                    Double_t dm2_41);
...
public:
  ...
  Double_t EvisToEnuMatrix(Int_t ievis, Int_t ienu)
  {
    return M_evis_to_enu[ievis][ienu];
  }
...
private:
  ...
  Double_t M[MaxPredictions * Nstage * max_n_evis_bins]
            [MaxPredictions * Nstage *
             max_n_evis_bins]; // covariance matrix for the fit
  Double_t M_scaled[MaxPredictions * Nstage * max_n_evis_bins]
                   [MaxPredictions * Nstage *
                    max_n_evis_bins]; // scaled covariance matrix for the fit
  Double_t M_inv[MaxPredictions * Nstage * max_n_evis_bins]
                [MaxPredictions * Nstage *
                 max_n_evis_bins]; // inverted covariance matrix for the fit
  ...
  Double_t M_bg_sys[MaxPredictions * Nstage * max_n_evis_bins]
                   [MaxPredictions * Nstage * max_n_evis_bins];
  Double_t M_bg_sys_frac[MaxPredictions * Nstage * max_n_evis_bins]
                        [MaxPredictions * Nstage * max_n_evis_bins];
  Double_t M_stat[MaxPredictions * Nstage * max_n_evis_bins]
                 [MaxPredictions * Nstage * max_n_evis_bins];
  Double_t
  ...
  Double_t NearEH_coeff[Nstage][Ndetectors]; // coefficients for taking weighted
                                             // sum of EH1+EH2
  Double_t FarAD_stage_coeff[Nstage][Ndetectors]; // coefficients for taking
                                                  // weighted sum of Far AD over
                                                  // different periods
  Double_t FarEH_stage_coeff[Nstage][Ndetectors]; // coefficients for taking
                                                  // weighted sum of Far EH over
                                                  // different periods
  ...
  Double_t final_covmatrix[MaxPredictions * Nstage * max_n_evis_bins *
                           MaxPredictions * Nstage * max_n_evis_bins];
  ...
  Double_t final_covmatrix_sum[MaxPredictions * max_n_evis_bins *
                               MaxPredictions * max_n_evis_bins];

  Double_t stat_factor[Nstage]; // factor to artificailly increase statitstics,
                                // by reducing stat error by 1/sqrt(stat_factor)
  Double_t days[Nstage];

  Double_t extra_days_8ad; // extra days for 8AD data taking period to reduce
                           // stat error
  Double_t days_6ad;       //
  Double_t days_8ad;       //
  Double_t days_7ad;       //

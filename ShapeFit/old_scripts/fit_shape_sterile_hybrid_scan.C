...
Int_t PeriodFlag = -1;//(0=6AD, 1=8AD, -1=6+8AD)
...
void fit_shape_sterile_hybrid_scan(bool isMC = false,
				   ...
				   Char_t savefilename[256] = "./fit_result_files/fit_shape_2d.root",
				   TString sig_spectra_filename0 = "./Spectra/ToyMC_6AD.root",
				   TString sig_spectra_filename1 = "./Spectra/ToyMC_8AD.root",
				   Bool_t logscale_dm41 = true,
  ...
  pred = new Predictor();
   ...
   FluxCalculator* fluxcalc = new FluxCalculator("./Distances/unblinded_baseline.txt","./Flux/SuperHistograms_P14A_unblinded_fine_huber-french.root");//<--flux calculator in super-hist mode 

  pred->EnterFluxCalculator(fluxcalc);

  Char_t Theta13InputsLocation[2][1024] = {"./Inputs/Theta13-inputs_p14a_inclusive_6ad.txt","./Inputs/Theta13-inputs_p14a_inclusive_8ad.txt"};
  ...
  TString AccidentalSpectrumLocation[2] = {"./Spectra/accidental_eprompt_shapes_6ad.root","./Spectra/accidental_eprompt_shapes_8ad.root"};
  ...
  */
  ...
  Double_t s22t13start=0.09;
  Double_t s22t13end=0.09;

  const Int_t nsteps_dm2 = 1;
  Double_t dm2eestart=2.41e-3;
  Double_t dm2eeend=2.41e-3;
  ...
  oscprobtab->SetDM41Range(nsteps_dm214,dm214start,dm214end,logscale_dm41);
  ...
  if (logscale_dm41) ranger_dm41->setLogScale();
  ...
  Double_t s2t_true = 0.084;
  Double_t dm2_true = 2.41e-3;
  Double_t s2t14_true = 0.0;
  Double_t dm241_true = 0.001;

  
  Double_t chi2result[nsteps_dm2][nsteps][nsteps_dm214_all][nsteps_s22t14];
  Double_t dchi2result[nsteps_dm2][nsteps][nsteps_dm214_all][nsteps_s22t14];
  Double_t sin22t13[nsteps_dm2][nsteps];
  Double_t dm2ee[nsteps_dm2][nsteps];

  Double_t chi2_true = 1e6;
  ...
  Double_t dm241_ini[nsteps_dm214_all];

  Double_t s22t13_min_local[nsteps_dm214_all];
  Double_t dm2ee_min_local[nsteps_dm214_all];
  Double_t s22t14_min_local[nsteps_dm214_all];
  Double_t dm241_min_local[nsteps_dm214_all];

  Double_t chi2_min_local[nsteps_dm214_all];
  Int_t fit_status[nsteps_dm214_all];

  
  Double_t s22t14_bins[nsteps_s22t14+1];
  Double_t dm214_bins[nsteps_dm214_all+1];
  
  for(int step_dm214=0;step_dm214<nsteps_dm214_all+1;++step_dm214){
    dm214_bins[step_dm214]=exp(log(dm214end/dm214start)*((Double_t)step_dm214-0.5)*1./(nsteps_dm214_all-1)+log(dm214start));
  }
  ...
  //  TH2F *h_chi2_all = new TH2F("h_chi2_all","",nsteps_s22t14,s22t14start,s22t14end,nsteps_dm214_all,dm214start,dm214end);
  // TH2D *h_chi2_all = new TH2D("h_chi2_all","",nsteps_s22t14,s22t14_bins,nsteps_dm214_all,dm214_bins);
  ...
  tr->Branch("chi2_map",&dchi2result[0][0][0][0],Form("chi2_map[%d][%d][%d][%d]/D",nsteps_dm2,nsteps,nsteps_dm214_all,nsteps_s22t14)); 
  ...
  tr->Branch("dm241_ini",&dm241_ini[0],Form("dm241_ini[%d]/D",nsteps_dm214_all)); 

  tr->Branch("s22t13_min_local",&s22t13_min_local[0],Form("s22t13_min_local[%d]/D",nsteps_dm214_all)); 
  tr->Branch("dm2ee_min_local",&dm2ee_min_local[0],Form("dm2ee_min_local[%d]/D",nsteps_dm214_all)); 
  tr->Branch("s22t14_min_local",&s22t14_min_local[0],Form("s22t14_min_local[%d]/D",nsteps_dm214_all)); 
  tr->Branch("dm241_min_local",&dm241_min_local[0],Form("dm241_min_local[%d]/D",nsteps_dm214_all)); 
  tr->Branch("chi2_min_local",&chi2_min_local[0],Form("chi2_min_local[%d]/D",nsteps_dm214_all)); 
  tr->Branch("fit_status",&fit_status[0],Form("fit_status[%d]/I",nsteps_dm214_all)); 

  tr->Branch("chi2_true",&chi2_true,"chi2_true/D"); 
  
  // TH2D* h_chi2_map = new TH2D("h_chi2_map","h_chi2_map",
  //                             nsteps_s22t14,s22t14_bins,nsteps_dm214_all,dm214_bins);
  ...
  for(int ievt=0;ievt<Nevts;++ievt){
    ...
    pred->FixCovMatrix(0.084,2.41e-3,0,0.1e-3,PeriodFlag);
    ...
    for(int step_dm214=0;step_dm214<nsteps_dm214_all;++step_dm214){
      ...
      //      dm214=(dm214end-dm214start)*step_dm214*1./(nsteps_dm214_all-1)+dm214start;
      ...
      s22t13_min_local[step_dm214] = pars[0];
      dm2ee_min_local[step_dm214] = pars[1];
      s22t14_min_local[step_dm214] = pars[2];
    ...
    //         dm214=(dm214end-dm214start)*step_dm214*1./(nsteps_dm214_all-1)+dm214start;
    //         // log scale
    //         //            dm214=exp(log(dm214end/dm214start)*step_dm214*1./(nsteps_dm214_all-1)+log(dm214start));  


    //         chi2result[step_dm2][step][step_dm214][step_s22t14]
    //           = oscprobtab->CalculateChi2CovQuick(sin22t13[step_dm2][step],dm2ee[step_dm2][step],sin22t14,dm214);
    //         // chi2result[step_dm2][step][step_dm214][step_s22t14]
    //         //   = pred->CalculateChi2Cov(sin22t13[step_dm2][step],dm2ee[step_dm2][step],sin22t14,dm214);
            
    //         //            cout << "Chi2 = " << chi2result[step_dm2][step][step_dm214][step_s22t14] << endl;
    //         if (chi2result[step_dm2][step][step_dm214][step_s22t14] < minchi2){
    //           minchi2 = chi2result[step_dm2][step][step_dm214][step_s22t14];
    //           bests22t13 = sin22t13[step_dm2][step];
    //           bestdm2ee = dm2ee[step_dm2][step];
    //           bests22t14 = sin22t14;
    //           bestdm214 = dm214;
    ...
    Double_t best_pars[4] = {bests22t13,bestdm2ee,bests22t14,bestdm214};
    
    cout << "======== fit results (for checking) :"
         << " " <<  bests22t13 << " " << bestdm2ee 
    ...
    chi2_true  = pred->CalculateChi2Cov(s2t_true,dm2_true,s2t14_true,dm241_true,PeriodFlag);
  ...
  tmp_set = pred->MakeOneSuperPrediction(bests22t13,bestdm2ee,bests22t14,bestdm214,true);
  ...
  cout << "Best delta m2(31): " << bestdm2ee << endl;
  ...
  for (Int_t iMode = 0; iMode < nModes; iMode++){
    ...
    tmp_vector = pred->GetFinalCovMatrixSum(-1,iMode);
    for (Int_t i = 0; i < n_rebinned_evis_bins; i++){
      for (Int_t j = 0; j < n_rebinned_evis_bins; j++){
        final_covmatrix_sum[iMode][i][j] += tmp_vector[i*n_rebinned_evis_bins+j];
      }
    }
  ...
  c2->Print("./fit_result_files/fit_shape_sterile_hybrid_scan.pdf["); //Open PDF file
  ...
  for (Int_t iMode = 0; iMode < nModes; iMode++){
	...
	h_final_obs_ratio[istage][iMode][iPred]->Draw("e same");
        
      }
      
      c2->Print("./fit_result_files/fit_shape_sterile_hybrid_scan.pdf");
    }
    ...
    for (Int_t iPred = 0; iPred <nPredictions[iMode]; iPred++){
      ...
      h_final_obs_ratio_sum[iMode][iPred]->Draw("e same");
    ...
    c3->Print("./fit_result_files/fit_shape_sterile_hybrid_scan.pdf");
    
  }

  c2->Print("./fit_result_files/fit_shape_sterile_hybrid_scan.pdf]"); //Close PDF file

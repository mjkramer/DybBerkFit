...
#include "PredSet.h"
// This object holds all the data for one time period (observed events,
// livetimes, muon efficiencies, backgrounds... etc).
...
public:
  ...
  ~TimePeriodData();
  void CorrectEvts(
      bool print = false); //<---correct for bg, livetimes and muon efficiencies
  void CorrectSpec(
      bool print = false); //<---correct for bg, livetimes and muon efficiencies
  void PrintToScreen();

  Double_t Livetime[Ndetectors] =
      {}; //<--in days? (doesn't matter as long as use same units)
  Double_t MuonVetoEff[Ndetectors] =
      {}; //<--muon veto efficiency (percent of time that did not cut out?)
  ...
  // Note to self: do not define as many histograms for bg as have equivalents
  // for numbers above for simplicity and in order to not have to clone them a
  // lot.
